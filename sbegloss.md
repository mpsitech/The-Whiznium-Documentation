...
block|engine, operation engine, clients|(de-)serializable (to/from XML) object used e.g. for settings and communication|
...
call listener|engine|feature attached to a job specifying to which calls it reacts with fine-grained masking options|
...
dialog|engine, HMI clients|optionally wizard-style sub-element to card, typically to manage a complex action / workflow - as opposed to simple view and control functionality handled in panels|
dispatch|engine, operation engine, clients|XML message, optionally container for multiple blocks|
dispatch collector|engine|element per session or per card which stores an arbitrary number of unsent engine-to-app dispatches while there is no request from the app. Updates of dispatch content handled by the engine's exchange object|
...
feed|HMI client|array data source to e.g. pop-up/list controls containing numeric and string references along with names|
...
input files|project configuration|the import/export complexes which are used to provide WhizniumSBE with a project's model information|
insertion point|engine, operation engine, HMI client|comment in source code indicating a spot into which code can be inserted manually|
